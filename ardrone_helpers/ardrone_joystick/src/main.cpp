...
struct TeleopArDrone
{
  ...
  ros::Publisher pub_takeoff, pub_land, pub_toggle_state, pub_vel, pub_vel2;
  ...
  std_srvs::Empty srv_empty;
    ...
    if (joy_msg->buttons.size() != 19 || joy_msg->axes.size() != 28){
	 ...
	 twist.linear.z = scale*joy_msg->axes[2]; // up down
	 twist.angular.z = scale*joy_msg->axes[3]; // yaw

	 twist2.linear.x = scale*joy_msg->axes[5]; // forward, backward
	 twist2.linear.y = scale**joy_msg->axes[4]; // left right
	 twist2.angular.z = scale*-1*joy_msg->buttons.at(0); // +yaw
	 twist2.angular.z = scale*joy_msg->buttons.at(1);// -yaw

	 // button 4 (L1): dead man switch
	 bool dead_man_pressed = joy_msg->buttons.at(4);

	 // button 5 (R1): switch emergeny state 
	...
	TeleopArDrone(){
	 ...
	 twist2.linear.x = twist2.linear.y = twist2.linear.z = 0;
	 twist2.angular.x = twist2.angular.y = twist2.angular.z = 0;
	 ...
	 pub_vel2           = nh_.advertise<geometry_msgs::Twist>("/4WMRobot/cmd_vel",1);
	 srv_cl_cam        = nh_.serviceClient<std_srvs::Empty>("/ardrone/togglecam",1);
	}

	void send_cmd_vel(){
    pub_vel.publish(twist);
    pub_vel2.publish(twist2);
  }
